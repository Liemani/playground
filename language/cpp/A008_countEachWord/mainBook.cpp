...
int main(int argc, char** argv) {
	...
	copy(wordArray.begin(),
			wordArray.end(),
			std::insert_iterator<StringSet>(wordSet, wordSet.begin()));

	WordCountMap wordCountMap;
	for (StringSet::iterator it = wordSet.begin(); it != wordSet.end(); ++it)
		wordCountMap.insert(WordCountPair(*it, count(wordArray.begin(), wordArray.end(), *it)));

...
void mainLoop(int kqueueFD, int serverSocketFD) {
    ...
    while (true) {
        ...
        for (int i = 0; i < eventCount; ++i) {
            struct kevent* const event = eventArray + i;
            const int fd = event->ident;
            const int16_t filter = event->filter;
            ...
            if (fd == serverSocketFD) {
                ...
                modifyEvent(kqueueFD, clientSocket, EVFILT_WRITE, EV_ADD);
            }
            else if (filter == EVFILT_WRITE) {
                char buf[] = "hi";
                ssize_t result = send(fd, buf, 2, 0);
                printf("result: [%ld]\n", result);
            }
        }
    }
